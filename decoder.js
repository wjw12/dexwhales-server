...
export default class Decoder {
    ...
    _addABI(abiArray) {
      ...
      if (Array.isArray(abiArray)) {
        ...
        self.state.savedABIs = self.state.savedABIs.concat(abiArray);
      } else {
      ...
      if (Array.isArray(abiArray)) {
        ...
        abiArray.map(function(abi) {
          if (abi.name) {
            ...
            if (abi.type === "event") {
              if (self.state.methodIDs[signature.slice(2)]) {
                delete self.state.methodIDs[signature.slice(2)];
              }
            } else {
              if (self.state.methodIDs[signature.slice(2, 10)]) {
                delete self.state.methodIDs[signature.slice(2, 10)];
              }
            }
          }
        });
    ...
    _decodeMethod(data) {
      ...
      if (abiItem) {
        ...
        throw new Error("methodID not found " + methodID);
      }
    }
